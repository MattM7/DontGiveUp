class LevelBase {
  ...
  ArrayList <Box> alPlat = new ArrayList<Box>(); // create a new ArrayList
  LevelBase() {
  ...
  void checkUpDown() {
    ...
    if (LvlBase.alPlat.size()!=0) {
      for (nI = 0; nI<LvlBase.alPlat.size(); nI++) {
        //isHitUpDown(float nX1, float nY1, float nX2, float nY2, int nH1, int nW1, int nH2, int nW2) {
        if (isHitBoxes(hero.vPos.x, hero.vPos.y, LvlBase.alPlat.get(nI).fX, LvlBase.alPlat.get(nI).fY, hero.imgHero.height, hero.imgHero.width, LvlBase.alPlat.get(nI).imgBox.height, LvlBase.alPlat.get(nI).imgBox.width)) {
          hero.fVelocity = 0; // reset velocity
          hero.nJumpCount=0; // if you hit the top or bottom of the box, it resets the jump amount
          hero.vPos.y = hero.vPosStart.y;
        }
