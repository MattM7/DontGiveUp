class Sprite {
  ...
  PVector vDirBullet;
  // ============== CONSTRUCTOR =============================================
  Sprite(float fTempX, float fTempY, float fTempAccel, float fTempVelocity, int nTempVelocityLimit, int nTempDirec, String sTempImgName, int nTempMinX, int nTempMaxX, int nTempMinY, int nTempMaxY, int nTempSpeed) {
    fX = fTempX;
    fY = fTempY;    
    fXstart = fX;
    fYstart = fY;
  ...
  void respawn() {
    ...
    nDeathCount++;
    refreshCoord();
    if (LvlBase.alGore.size()>10) { // you see, we would have used an array for alGore, but the naming of the arrayList 'alGore" is just too precious.
      LvlBase.alGore.remove(0);
    }
    LvlBase.alGore.add(new Sprite(fX, fY, 0, 0, 0, 0, "alGore.png", 0, 0, 0, 0, 0));
    ...
    refreshCoord();
    if (nLevel==4) {
      messageEye.sMessage[3] = "You betrayed me "+names.sName;
    }
  }
  // ============== DISPLAY =============================================
  void display() {
