class Sprite {
  ...
  boolean bHasTimerStarted = false; // used to delay things (like falling platforms, moving spikes, and moving onto the next level)
  boolean bChangeDir;
  PImage img;
  int nDeathCount;
  ...
  void respawn() {
    ...
    nDeathCount++;
    refreshCoord();
    if (LvlBase.alGore.size()>5) { // you see, we would have used an array for Al Gore images, but the naming of the arrayList 'alGore" is just too precious.
      LvlBase.alGore.remove(0);
    }
    LvlBase.alGore.add(new Sprite(fX, fY, 0, 0, 0, 0, "alGore.png", 0, 0, 0, 0, false));
  ...
  void jump() {
    if (nJumpCount==0) { // only let's you jump if you haven't jumped yet
      ...
      if (bFlipGravity) {
        ...
        fVelocity = -21;
      } // jump // to fix the lag had to change from 15 to 21.
    }
  }
  ...
  void updateMovingSpikes() {
    for (Sprite nI : LvlBase.alBox) {
      if (LvlBase.isHit(this, nI)) {
        round(fY);
