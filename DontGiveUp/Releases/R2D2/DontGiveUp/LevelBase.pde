...
class LevelBase {
  ...
  void checkForHitDoors() {
    if (isHit(sprHero, entry)) {
    } else if (isHit(sprHero, exit)) {
  ...
  void updateBullets() {
    if (alBullets.size()!=0) {
      for (int nI = alBullets.size()-1; nI>=0; nI--) {
        ...
        if (!bSkip) {
          if (alBullets.size()!=0) {
            for (int nM = 0; nM<alFallPlats.size(); nM ++) {
              if (isHit(alBullets.get(nI), alFallPlats.get(nM))) {
                alBullets.remove(nI);
                bSkip=true;
                //nI--;
                break;
        ...
        if (!bSkip) {
          if (alBullets.size()!=0) {
            for (int nN = 0; nN<alBox.size(); nN ++) {
              if (isHit(alBullets.get(nI), alBox.get(nN))) {
                alBullets.remove(nI);
                bSkip=true; // just in case we decide to put anythign after this
                //nI--;
                break;
              }
            }
  ...
  // ============== IS NEAR =============================================
  boolean isNear(Sprite one, Sprite two, int nRange) { // needed for  animating the door later on
    float fX1, fY1, fX2, fY2;
    int nH1, nW1, nH2, nW2;
    fX1 = one.fX-nRange;
    fY1 = one.fY-nRange; 
