// Release 4.1 -Ewok - Lasers!
//  lags, check frameRate?
...
void setup() {
  ...
  entry = new Sprite (nBoxSize, nLevelHeight-nBoxSize-75, 0, 0, 0, 0, "entryDoor.png"); //75 for height of the door 
  exit = new Sprite (nLevelWidth-nBoxSize-48, nLevelHeight-nBoxSize-75, 0, 0, 0, 0, "exitDoor.png"); // 48 for width of the door
  hero = new Sprite (75, 75, 1.6, 0.6, 16, 0, "PixelCrab.png"); //construct a hero // to fix the lag had to change from 0.8 to 1.6, 0.3 to 0.6, 8 to 16 
  ...
  buttonTimer = new Timer(3000); // just choose 3000 as a random tim, it is later set to the length of the audio
  ...
  while (giveUpButton.nCount<=9) {
    ...
    // println(soundMessage[giveUpButton.nCount].getGain()); 
    giveUpButton.nCount++;
  }
  giveUpButton.nCount=0;

  laserGun = new LaserGun(nBoxSize*5, nBoxSize*3);
...
void nullAndRecreatePlatAL() {
  ...
  laserGun.alBullets = null;
  laserGun.alBullets = new ArrayList <Sprite> ();
}
// ============== STOP =============================================
void stop() {
  giveUpButton.nCount=0;
